...
j <-  1987 # define the interator

while (j <= 2002) {
  sd_le <- sd(gapminder$lifeExp[gapminder$year == j], na.rm=T)
  print(paste0(j, ": ", sd_le)
        )
  j <- j + 1 # increase the iterator by the interval between years
}
```
...
for (i in unique(gapminder$year)) {
  ...
  if (i>=1987){
    print (paste("Mean Population of", i, "is", tmp))
  }
  else{
    print (paste("Year", i, "is less than 1987"))
  }
}
...
report_mean_sd <- 
  function(df, variable, country) {new_data<-data[is.na(data$Ozone))]
...
- Hint: `min`, `max`
```{r}
min_max <- function(df, variable, continent) {
  new <- df[variable][df$continent==continent]
  new_min<-min(new)
  new_max<-max(new)
  new_mean<-mean(new)
  new_median<-median(new)
  cat("Continent:", continent, 
        "\nMin Life Expectancy:", new_min,
        "\nMax Life Expectancy:", new_max,
        "\nMean Life Expectancy:", new_mean,
        "\nMedian:", new_median
      )
}

min_max(gapminder, "lifeExp", "Asia")
## df[[variable]]: using with a string, can also be used as df[, variable]
```
